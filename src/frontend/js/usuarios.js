...
function getRoleName(level) {
    if (level === 1) return '<span class="role-badge role-1">🔵 Cajero</span>';
    if (level === 2) return '<span class="role-badge role-2">🟢 Administrativo</span>';
    if (level === 3) return '<span class="role-badge role-3">🏆 Corporativo</span>';
    return '<span class="role-badge">Desconocido</span>';
}
            ...
            row.innerHTML = `
                ...
                <td>
                    <div style="display:flex;gap:4px;justify-content:center;">
                        <button class="btn-act btn-act-edit" onclick="openEditModal('${user.id}', '${user.name}', '${user.email}', '${user.role_level}')">✏️ Editar</button>
                        ${user.email !== 'admin@cierrex.com' ? `<button class="btn-act btn-act-delete" onclick="deleteUser('${user.id}')">🗑️ Eliminar</button>` : '<span style="font-size:0.7rem;color:rgba(255,255,255,0.4);display:flex;align-items:center;padding:0 5px;">Admin Fijo</span>'}
                    </div>
                </td>
            `;
