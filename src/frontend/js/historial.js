    ...
    try {
        ...
        filteredCierres.forEach(cierre => {
            ...
            if (userIsAdmin) {
                actionButtonsHtml = `
                    <div class="action-buttons-container">
                        <button class="btn-act btn-act-pdf" onclick="recreatePdf('${cierre.id}')">📄 PDF</button>
                        <button class="btn-act btn-act-edit" onclick="openEditCierreModal('${cierre.id}')">✏️ Editar</button>
                        <button class="btn-act btn-act-delete" onclick="deleteCierre('${cierre.id}')">🗑️ Eliminar</button>
                    </div>
                `;
            }
            ...
            row.innerHTML = `
                ...
                <td>${cierre.image ? `<img src="${cierre.image}" class="img-thumb" onclick="showImage('${cierre.image}')" title="Ver comprobante">` : '<span style="color:#94A3B8;">—</span>'}</td>
