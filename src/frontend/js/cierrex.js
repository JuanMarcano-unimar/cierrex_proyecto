        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                if (user.role_level > 1) {
                    ...
                    ['efectivo','debito','pagoMovil','divisas'].forEach(id => {
                        const el = document.getElementById(id);
                        if (el) el.value = '';
                    });
                    // Ocultar extras de nuevo
                    ['credito','transferencias','zelle'].forEach(id => {
                        const el = document.getElementById(id);
                        if (el) el.value = '';
                    });
                    document.getElementById('extraPayments').style.display = 'none';
                    document.getElementById('toggleExtraBtn').style.display = '';
                    document.getElementById('zelleWrapper').style.display = 'none';
                    document.getElementById('zelleToggleWrapper').style.display = 'flex';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (toggleExtraBtn) {
        toggleExtraBtn.addEventListener('click', () => {
            document.getElementById('extraPayments').style.display = 'block';
            toggleExtraBtn.style.display = 'none';
        });
    }

    const toggleZelleBtn = document.getElementById('toggleZelleBtn');
    if (toggleZelleBtn) {
        toggleZelleBtn.addEventListener('click', () => {
            document.getElementById('zelleWrapper').style.display = 'block';
            document.getElementById('zelleToggleWrapper').style.display = 'none';
        });
    }
});
