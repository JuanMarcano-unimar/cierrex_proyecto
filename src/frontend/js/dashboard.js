...
async function fetchDailyCierre() {
    ...
    try {
        ...
        document.getElementById('dailyTotalUsd').textContent = `$${totalUsd.toFixed(2)}`;
        document.getElementById('dailyTotalVes').textContent = `Bs ${totalVes.toFixed(2)}`;
        const countEl = document.getElementById('dailyCierresCount');
        if (countEl) countEl.textContent = dailyCierres.length;

        document.getElementById('dailyDetails').innerHTML = [
            { lbl: '💵 Efectivo',       val: `Bs ${details.efectivo.toFixed(2)}` },
            { lbl: '💳 Débito',         val: `Bs ${details.debito.toFixed(2)}` },
            { lbl: '🏦 Crédito',        val: `Bs ${details.credito.toFixed(2)}` },
            { lbl: '📱 Pago Móvil',     val: `Bs ${details.pagoMovil.toFixed(2)}` },
            { lbl: '🔄 Transferencias', val: `Bs ${details.transferencias.toFixed(2)}` },
            { lbl: '💵 Divisas',        val: `$${details.divisas.toFixed(2)}` },
            { lbl: '💚 Zelle',          val: `$${details.zelle.toFixed(2)}` },
        ].map(d => `
            <div class="detail-item">
                <div class="d-lbl">${d.lbl}</div>
